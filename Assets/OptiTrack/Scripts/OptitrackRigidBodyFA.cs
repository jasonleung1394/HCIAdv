...
/// </summary>
public class OptitrackRigidBodyFA : MonoBehaviour
{
    ...
    /// <summary>
    ///  As an add-on feature, the rbstate have two method for constructing the forearm orientation.
    /// Tho the Quaternion.Inverse suffers from sudden movement from the shoulder.
    /// </summary>
    void UpdatePose()
    {
        OptitrackRigidBodyState rbState = StreamingClient.GetLatestRigidBodyState(RigidBodyId, NetworkCompensation);
        OptitrackRigidBodyState rbState_Arm = StreamingClient.GetLatestRigidBodyState(3, NetworkCompensation);
        ...
        rbState_Arm.Pose.Orientation = new Quaternion(-rbState_Arm.Pose.Orientation.y, -rbState_Arm.Pose.Orientation.x, -rbState_Arm.Pose.Orientation.z, rbState_Arm.Pose.Orientation.w);


        if (rbState != null)
        {
            this.transform.localPosition = rbState.Pose.Position;
            // this.transform.localRotation = rbState.Pose.Orientation * Quaternion.Inverse(rbState_Arm.Pose.Orientation);
        }
    }
}
